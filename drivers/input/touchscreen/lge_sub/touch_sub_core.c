...
#include <linux/irq.h>
#include <linux/input/lge_touch_sub_notify.h>
...
{
	struct touch_sub_core_data *ts = plist_sub->touch_sub_core_data;

	TOUCH_TRACE();

	if (ts == NULL) {
		TOUCH_I("%s - ts is NULL\n", __func__);
		return;
	}

	if (on_off) {
		TOUCH_I("%s - secure touch of main panel is enabled, set touch_disable_irq of sub panel!\n", __func__);
		touch_sub_interrupt_control(ts->dev, INTERRUPT_DISABLE);
	} else {
		TOUCH_I("%s - secure touch of main panel is disabled, set touch_enable_irq of sub panel!\n", __func__);
...
void touch_sub_report_event(struct touch_sub_core_data *ts)
{
	...
	if (ts->is_cancel) {
		touch_sub_report_cancel_event(ts);
		ts->is_cancel = 0;
	}
...
void touch_sub_report_all_event(struct touch_sub_core_data *ts)
{
	TOUCH_TRACE();
	...
	if (ts->old_mask) {
		...
		ts->tcount = 0;
		memset(ts->tdata, 0, sizeof(struct touch_sub_data) * MAX_FINGER);
	}
	ts->is_cancel = 0;
}

static void touch_sub_core_initialize(struct touch_sub_core_data *ts)
{
	/* lockscreen */
	touch_sub_report_all_event(ts);
}

irqreturn_t touch_sub_irq_handler(int irq, void *dev_id)
{
	struct touch_sub_core_data *ts = (struct touch_sub_core_data *) dev_id;
...
irqreturn_t touch_sub_irq_thread(int irq, void *dev_id)
{
	struct touch_sub_core_data *ts = (struct touch_sub_core_data *) dev_id;
	int ret = 0;
	...
	if (ret >= 0) {
		...
		if (ts->intr_status & TOUCH_IRQ_PASSWD)
			touch_sub_send_uevent(ts, TOUCH_UEVENT_PASSWD);

		if (ts->intr_status & TOUCH_IRQ_SWIPE_DOWN)
			touch_sub_send_uevent(ts, TOUCH_UEVENT_SWIPE_DOWN);

		if (ts->intr_status & TOUCH_IRQ_SWIPE_UP)
			touch_sub_send_uevent(ts, TOUCH_UEVENT_SWIPE_UP);

		if (ts->intr_status & TOUCH_IRQ_SWIPE_LEFT)
			touch_sub_send_uevent(ts, TOUCH_UEVENT_SWIPE_LEFT);

		if (ts->intr_status & TOUCH_IRQ_SWIPE_RIGHT)
			touch_sub_send_uevent(ts, TOUCH_UEVENT_SWIPE_RIGHT);

		if (ts->intr_status & TOUCH_IRQ_WATER_MODE_ON)
			touch_sub_send_uevent(ts, TOUCH_UEVENT_WATER_MODE_ON);

		if (ts->intr_status & TOUCH_IRQ_WATER_MODE_OFF)
			touch_sub_send_uevent(ts, TOUCH_UEVENT_WATER_MODE_OFF);

		if (ts->intr_status & TOUCH_IRQ_AI_BUTTON)
			touch_sub_send_uevent(ts, TOUCH_UEVENT_AI_BUTTON);

		if (ts->intr_status & TOUCH_IRQ_AI_PICK)
			touch_sub_send_uevent(ts, TOUCH_UEVENT_AI_PICK);

		if (ts->intr_status & TOUCH_IRQ_SWIPE_LEFT2)
			touch_sub_send_uevent(ts, TOUCH_UEVENT_SIDE_PAY);

		if (ts->intr_status & TOUCH_IRQ_SWIPE_RIGHT2)
			touch_sub_send_uevent(ts, TOUCH_UEVENT_SIDE_PAY);

		if (ts->intr_status & TOUCH_IRQ_LPWG_LONGPRESS_DOWN)
			touch_sub_send_uevent(ts, TOUCH_UEVENT_LPWG_LONGPRESS_DOWN);

		if (ts->intr_status & TOUCH_IRQ_LPWG_LONGPRESS_UP)
			touch_sub_send_uevent(ts, TOUCH_UEVENT_LPWG_LONGPRESS_UP);

		if (ts->intr_status & TOUCH_IRQ_LPWG_SINGLE_WAKEUP)
			touch_sub_send_uevent(ts, TOUCH_UEVENT_LPWG_SINGLE_WAKEUP);

		if (ts->intr_status & TOUCH_IRQ_PEN_WAKEUP)
			touch_sub_send_uevent(ts, TOUCH_UEVENT_PEN_WAKEUP);

		if (ts->intr_status & TOUCH_IRQ_PEN_WAKEUP_BTN)
			touch_sub_send_uevent(ts, TOUCH_UEVENT_PEN_WAKEUP_BTN);
...
static void touch_sub_init_work_func(struct work_struct *init_work)
{
	struct touch_sub_core_data *ts =
		container_of(to_delayed_work(init_work),
				struct touch_sub_core_data, init_work);
	int ret = 0;

	TOUCH_TRACE();

	TOUCH_I("touch_sub_ic_init Start\n");
	mutex_lock(&ts->lock);
	touch_sub_core_initialize(ts);
	ret = ts->driver->init(ts->dev);
	if (ret < 0) {
		TOUCH_E("touch_sub_ic_init failed %d\n", ret);
		mutex_unlock(&ts->lock);
	...
	atomic_set(&ts->state.core, CORE_NORMAL);
	TOUCH_I("touch_sub_ic_init End\n");
}

static void touch_sub_upgrade_work_func(struct work_struct *upgrade_work)
{
	struct touch_sub_core_data *ts =
		container_of(to_delayed_work(upgrade_work),
				struct touch_sub_core_data, upgrade_work);
	int ret;
...
static void touch_sub_fb_work_func(struct work_struct *fb_work)
{
	struct touch_sub_core_data *ts =
			container_of(to_delayed_work(fb_work),
				struct touch_sub_core_data, fb_work);

	TOUCH_TRACE();

	if (atomic_read(&ts->state.fb) == FB_SUSPEND)
		touch_sub_suspend(ts->dev);
	else if (atomic_read(&ts->state.fb) == FB_RESUME)
		touch_sub_resume(ts->dev);
}

static void touch_sub_panel_reset_work_func(struct work_struct *panel_reset_work)
{
	struct touch_sub_core_data *ts =
			container_of(to_delayed_work(panel_reset_work),
				struct touch_sub_core_data, panel_reset_work);
...
static int touch_sub_check_driver_function(struct touch_sub_core_data *ts)
{
	if (ts->driver->probe &&
			ts->driver->remove &&
			ts->driver->shutdown &&
...
static int touch_sub_init_platform_data(struct touch_sub_core_data *ts)
{
	int ret;

	TOUCH_TRACE();

	ret = touch_sub_check_driver_function(ts);
	...
	if (ts->dev->of_node)
		ret = touch_sub_get_dts(ts);
	else
		ret = touch_sub_get_platform_data(ts);
...
static void touch_sub_init_locks(struct touch_sub_core_data *ts)
{
	TOUCH_TRACE();
...
static int touch_sub_init_input(struct touch_sub_core_data *ts)
{
...
static void touch_sub_suspend(struct device *dev)
{
	...
	struct module_data *md = to_module(plist_sub->sub_dev[0]);
	int ret = 0;
...
static void touch_sub_early_suspend(struct early_suspend *h)
{
	struct touch_sub_core_data *ts =
		container_of(h, struct touch_sub_core_data, early_suspend);
...
static void touch_sub_early_resume(struct early_suspend *h)
{
	struct touch_sub_core_data *ts =
		container_of(h, struct touch_sub_core_data, early_suspend);
...
static int touch_sub_init_pm(struct touch_sub_core_data *ts)
{
	...
	ts->early_suspend.suspend = touch_sub_early_suspend;
	ts->early_suspend.resume = touch_sub_early_resume;
	register_early_suspend(&ts->early_suspend);
	return 0;
}

#elif defined(CONFIG_DRM_MSM) && defined(CONFIG_FB)
static int touch_sub_drm_notifier_callback(struct notifier_block *self,
		unsigned long event, void *data)
{
	struct touch_sub_core_data *ts =
		container_of(self, struct touch_sub_core_data, drm_notif);
...
static int touch_sub_init_pm(struct touch_sub_core_data *ts)
{
	...
	ts->drm_notif.notifier_call = touch_sub_drm_notifier_callback;
	ts->driver->init_pm(ts->dev);
	return msm_drm_register_client(&ts->drm_notif);
}

#elif defined(CONFIG_FB)
static int touch_sub_fb_notifier_callback(struct notifier_block *self,
		unsigned long event, void *data)
{
	struct touch_sub_core_data *ts =
		container_of(self, struct touch_sub_core_data, fb_notif);
...
static struct bus_type touch_sub_uevent_subsys = {
	...
	.dev_name = LGE_TOUCH_SUB_NAME,
};

static struct device device_uevent_touch = {
	.id    = 0,
	.bus   = &touch_sub_uevent_subsys,
};

static int touch_sub_init_uevent(struct touch_sub_core_data *ts)
{
	int ret = 0;

	TOUCH_TRACE();

	ret = subsys_system_register(&touch_sub_uevent_subsys, NULL);
	if (ret < 0)
...
static char *uevent_str[TOUCH_UEVENT_SIZE][2] = {
...
void touch_sub_send_uevent(struct touch_sub_core_data *ts, int type)
{
	int ret = 0;
...
void touch_sub_notify_swivel(u32 type)
{
	swivel = type;
	touch_sub_atomic_notifier_call(NOTIFY_SWIVEL, &type);
}
EXPORT_SYMBOL(touch_sub_notify_swivel);
...
void touch_sub_restore_state(struct touch_sub_core_data *ts) {
	...
	if (atomic_read(&ts->state.swivel) != swivel) {
		...
		touch_sub_notify_swivel(swivel);
	}
}
				   ...
				   unsigned long event, void *data)
{
	...
	boot_mode = touch_sub_check_boot_mode(ts->dev);
	...
	if (ts->driver->notify) {
		...
		case NOTIFY_FM_RADIO:
			...
			break;
		case NOTIFY_SWIVEL:
			if (atomic_read(&ts->state.swivel) == SWIVEL_NOT_INITIALIZED) {
				atomic_set(&ts->state.prev_swivel, *(int *)data);
			} else {
				atomic_set(&ts->state.prev_swivel, atomic_read(&ts->state.swivel));
			}
...
static int display_notify(struct touch_sub_core_data *ts,
				   unsigned long event, void *data)
{
	int boot_mode = TOUCH_NORMAL_BOOT;
	int ret = 0;
...
static int touch_sub_blocking_notifier_callback(struct notifier_block *this,
				   unsigned long event, void *data)
{
	struct touch_sub_core_data *ts =
		container_of(this, struct touch_sub_core_data, blocking_notif);

	TOUCH_TRACE();

	return touch_sub_notify(ts, event, data);
}

static void touch_sub_atomic_notifer_work_func(struct work_struct *notify_work)
{
	struct touch_sub_core_data *ts =
		container_of(to_delayed_work(notify_work),
				struct touch_sub_core_data, notify_work);
	...
	for (i = 0; i < ATOMIC_NOTIFY_EVENT_SIZE; i++) {
		if (arr[i].event != NOTIFY_NO_EVENT) {
			touch_sub_notify(ts, arr[i].event, &(arr[i].data));
			arr[i].event = NOTIFY_NO_EVENT;
		}
	}
}

static int touch_sub_atomic_notifier_callback(struct notifier_block *this,
				   unsigned long event, void *data)
{
	struct touch_sub_core_data *ts =
		container_of(this, struct touch_sub_core_data, atomic_notif);
	struct atomic_notify_event *arr = ts->notify_event_arr;
...
{
	struct touch_sub_core_data *ts =
		container_of(this, struct touch_sub_core_data, display_notif);
...
static int touch_sub_init_notify(struct touch_sub_core_data *ts)
{
	int ret = 0;

	TOUCH_TRACE();

	ts->blocking_notif.notifier_call = touch_sub_blocking_notifier_callback;
	ret = touch_sub_blocking_notifier_register(&ts->blocking_notif);

	if (ret < 0)
		TOUCH_E("failed to regiseter touch blocking_notify callback\n");

	ts->atomic_notif.notifier_call = touch_sub_atomic_notifier_callback;
	ret = touch_sub_atomic_notifier_register(&ts->atomic_notif);
...
static int touch_sub_init_works(struct touch_sub_core_data *ts)
{
	ts->wq = create_singlethread_workqueue("touch_sub_wq");
...
#if defined(CONFIG_SUB_SECURE_TOUCH)
void secure_touch_sub_notify(struct touch_sub_core_data *ts)
{
	TOUCH_I("secure_touch_sub_notify\n");
	sysfs_notify(&ts->kobj, NULL, "secure_touch");
}

void secure_touch_sub_init(struct touch_sub_core_data *ts)
{
	ts->st_initialized = 0;
 ...
 *   active.
 */
void secure_touch_sub_stop(struct touch_sub_core_data *ts, bool blocking)
{
	TOUCH_I("secure_touch_sub_stop : %d", blocking);
	if (atomic_read(&ts->st_enabled)) {
...
static int module_notify_func(int control, int data) {
	switch (control) {
		case MODULE_FIRMWARE_UPGRADE:
			...
			notify_touch_sub_version_check(data);
#endif
			break;
		case 2:
			break;
...
static int touch_sub_core_probe_normal(struct platform_device *pdev)
{
	struct touch_sub_core_data *ts;
	int ret = 0;
	...
	/* sub module */
	if (plist_sub == NULL) {
		plist_sub = kzalloc(sizeof(*plist_sub), GFP_KERNEL);
		if (!plist_sub) {
			TOUCH_E("Failed to allocate memory for plist_sub\n");
			return -ENOMEM;
		}
	}
...
//		char buf[100] = {0};
		...
		plist_sub->sub_dev[pdev->id - 1] = pdev->dev.parent;
	...
	ts = (struct touch_sub_core_data *) pdev->dev.platform_data;
	plist_sub->touch_sub_core_data = (void *)ts;

	atomic_set(&ts->state.core, CORE_PROBE);
	ret = touch_sub_init_platform_data(ts);
	if (ret < 0) {
	...
	touch_sub_init_locks(ts);
	ret = touch_sub_init_works(ts);
	if (ret < 0) {
		TOUCH_E("failed to initialize works\n");
		goto error_init_work;
	}

	ret = touch_sub_init_input(ts);
	if (ret < 0) {
		TOUCH_E("failed to register input device(ret:%d)\n", ret);
	...
	ret = touch_sub_init_uevent(ts);
	ret = touch_sub_init_sysfs(ts);
...
static int touch_sub_core_probe_etc(struct platform_device *pdev)
{
	struct touch_sub_core_data *ts;
	int ret = 0;
...
static int touch_sub_core_probe(struct platform_device *pdev)
{
	struct touch_sub_core_data *ts;
	int boot_mode = TOUCH_NORMAL_BOOT;
	int ret = 0;
	...
	mutex_lock(&touch_sub_probe_lock);

	if (pdev->id > 0) {
		TOUCH_I("pdev->id = %d, no need to check bootmode\n", pdev->id);
		ret = touch_sub_core_probe_normal(pdev);
		goto out;
	}
	...
	if (boot_mode == TOUCH_CHARGER_MODE
			|| boot_mode == TOUCH_LAF_MODE
			|| boot_mode == TOUCH_RECOVERY_MODE) {
		ret = touch_sub_core_probe_etc(pdev);
		goto out;
	}
...
out:
	mutex_unlock(&touch_sub_probe_lock);
	return ret;
}

static int touch_sub_core_remove(struct platform_device *pdev)
{
	struct touch_sub_core_data *ts;
	...
	if (plist_sub != NULL && pdev->id > 0) {
		struct module_data *md = to_module(plist_sub->sub_dev[pdev->id - 1]);
		if (md != NULL)
			md->m_driver.remove(md->dev);

		plist_sub->sub_dev[pdev->id - 1] = NULL;
	...
	touch_sub_blocking_notifier_unregister(&ts->blocking_notif);

	touch_sub_atomic_notifier_unregister(&ts->atomic_notif);
...
static void touch_sub_core_shutdown(struct platform_device *pdev)
{
	struct touch_sub_core_data *ts;
...
#if defined(CONFIG_SUB_SECURE_TOUCH)
	if (atomic_read(&ts->st_enabled)) {
		secure_touch_sub_stop(ts, false);
...
static struct platform_driver touch_sub_core_driver = {
	.driver = {
		.name = LGE_TOUCH_SUB_DRIVER_NAME,
		.owner = THIS_MODULE,
/*
		.of_match_table = touch_sub_match_ids,
*/
	},
	.probe = touch_sub_core_probe,
	.remove = touch_sub_core_remove,
	.shutdown = touch_sub_core_shutdown,
};

static void touch_sub_core_async_init(void *data, async_cookie_t cookie)
{
	...
	mutex_init(&touch_sub_probe_lock);

	ret = platform_driver_register(&touch_sub_core_driver);

	if (ret < 0)
		TOUCH_E("async_init failed\n");
}

static int __init touch_sub_core_init(void)
{
	TOUCH_TRACE();
	async_schedule(touch_sub_core_async_init, NULL);
	return 0;
}

static void __exit touch_sub_core_exit(void)
{
	TOUCH_TRACE();
	platform_driver_unregister(&touch_sub_core_driver);
}

late_initcall(touch_sub_core_init);
module_exit(touch_sub_core_exit);
