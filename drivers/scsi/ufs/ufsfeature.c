...
{
	INIT_INFO("init start.. tw_state %d", atomic_read(&ufsf->tw_state));
...
{
	INIT_INFO("reset start.. tw_state %d", atomic_read(&ufsf->tw_state));
...
{
	INFO_MSG("run reset_lu.. tw_state %d", atomic_read(&ufsf->tw_state));
...
{
	INFO_MSG("run reset_host.. tw_state %d", atomic_read(&ufsf->tw_state));
	if (atomic_read(&ufsf->tw_state) == TW_PRESENT)
		atomic_set(&ufsf->tw_state, TW_RESET);
}
