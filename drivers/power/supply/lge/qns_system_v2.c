...
static bool qns_has_psy(void)
{
	...
	if (data.fg_psy == NULL) {
		data.fg_psy = power_supply_get_by_name("bms");
		if (data.fg_psy == NULL) {
			pr_qns(PR_ERROR, "ERROR: unable to get fg\n");
			return false;
		}
	}
...
static int qns_get_scvt(int *soc, int *c, int *v, int *tx10)
{
	...
	*/
	...
	if (data.battery_psy) {
		...
		if (soc != NULL) {
			/* when it uses POWER_SUPPLY_PROP_CAPACITY,
			   Charging time can increase,
			   because capacity reports faster than capacity_raw */
#ifdef CONFIG_LGE_PM_EXT_QG_V2
			if (power_supply_get_property(data.fg_psy,
					POWER_SUPPLY_PROP_CAPACITY_LEVEL, &ret) != 0) {
#else
			if (power_supply_get_property(data.battery_psy,
...
#endif
				pr_qns(PR_DEBUG, "read soc: %d(raw=%d)\n", *soc, ret.intval);
			}
		}
	} else {
