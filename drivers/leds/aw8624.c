...
#include <linux/aw8624.h>
#include <linux/aw8624_reg.h>
...
#define AW8624_F0_FILE "/mnt/vendor/persist-lg/aw8624/aw8624_f0"
...
static int aw8624_read_f0_file(const char *filename, char *buf, int len);
static void aw8624_write_f0_file(const char *filename, char *buf, int len);
static void aw8624_f0_cal_update(struct aw8624 *aw8624, int f0);
/******************************************************
 *
...
{
#ifdef OP_AW_DEBUG
	do_gettimeofday(&aw8624->t_stop);
	aw8624->game_microsecond = (aw8624->t_stop.tv_sec-aw8624->t_start.tv_sec) * 1000000
		+ (aw8624->t_stop.tv_usec-aw8624->t_start.tv_usec);
	pr_info("%s: haptic_audio_delay=%dus\n", __func__, aw8624->game_microsecond);
	if (aw8624->game_microsecond <= 2000) {
		mdelay(2);
	}
#endif
...
#ifdef OP_AW_DEBUG
	do_gettimeofday(&aw8624->t_start);
#endif
	return 0;
}
...
static int aw8624_haptic_ram_config(struct aw8624 *aw8624)
{
	...
	if ((aw8624->duration > 0) &&
		...
		wavseq = aw8624_dts_data.aw8624_wavseq[7];    /*#4  long vibration*/
	...
	pr_info("%s duration=%d wavseq=%d\n", __func__, aw8624->duration, wavseq);
...
static int aw8624_read_f0_file(const char *filename, char *buf, int len)
{
	...
	memset(buf, 0, len);
	vfs_read(fp, buf, len - 1, &pos);
	filp_close(fp, NULL);
	pr_info("%s: read complete\n", __func__);
exit:
	set_fs(fs);
	return ret;
}
...
static void aw8624_write_f0_file(const char *filename, char *buf, int len)
{
	...
	fp = filp_open(filename, O_WRONLY | O_CREAT, 0660);
	if (IS_ERR(fp)) {
		pr_err("%s: open error\n", __func__);
		ret = -EIO;
		goto exit;
...
static void aw8624_f0_cal_update(struct aw8624 *aw8624, int f0)
{
	...
	pr_info("%s : f0 value %d", __func__, f0);
...
static int aw8624_haptic_init(struct aw8624 *aw8624)
{
	...
	f0_p = strstr(saved_command_line,"androidboot.vendor.lge.f0_result=");
	if(f0_p != NULL){
		strncpy(f0_string, f0_p+33, 4);
		kstrtouint(f0_string, 0, &f0_receive);
		pr_info("%s: aw8624 f0_result = %d\n", __func__, f0_receive);
	}

	if (f0_receive != 0 && (f0_receive > 2515 || f0_receive < 2185)){
		pr_err("%s: Wrong value %d. use saved value.\n", __func__, f0_receive);
		f0_receive = 0;
	}
	aw8624->f0 = f0_receive;
	...
	if (f0_receive > 0)
		aw8624_f0_cal_update(aw8624, f0_receive);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u %u %u %u %u %u", &databuf[0], &databuf[1],
		...
		pr_debug("%s: cnt=%u, cmd=%u, play=%u, wavseq=%u, loop=%u, gain=%u\n",
			 __func__,
...
static ssize_t aw8624_f0_init_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct led_classdev *to_dev = dev_get_drvdata(dev);
	...
	if (aw8624->f0 == 0) {
		char f0_string[32] = {0};
		int f0_receive = 0;
		if (!aw8624_read_f0_file(AW8624_F0_FILE, f0_string, sizeof(f0_string) - 1)) {
			kstrtouint(f0_string, 0, &f0_receive);
			pr_info("%s: use saved f0 value %d\n", __func__, f0_receive);
		}
		if (f0_receive > 2515 || f0_receive < 2185){
			pr_err("%s: Saved calib data error as %d. \n", __func__, f0_receive);
			f0_receive = aw8624->f0_pre;
			pr_err("%s: Use default value %d \n", __func__, f0_receive);
		}
		aw8624_f0_cal_update(aw8624, f0_receive);
	} else {
		char f0_string[32] = {0};
		snprintf(f0_string, sizeof(f0_string) - 1, "%d", aw8624->f0);
		aw8624_write_f0_file(AW8624_F0_FILE, f0_string, strlen(f0_string));
	}
	mutex_unlock(&aw8624->lock);
...
static DEVICE_ATTR(f0_init, 0220, NULL, aw8624_f0_init_store);
#endif
...
static int aw8624_hw_reset(struct aw8624 *aw8624)
{
	...
	if (aw8624 && gpio_is_valid(aw8624->reset_gpio)) {
		gpio_direction_output(aw8624->reset_gpio, 0);
		usleep_range(1000, 2000);
		gpio_direction_output(aw8624->reset_gpio, 1);
...
static int aw8624_i2c_remove(struct i2c_client *i2c)
{
	...
	mutex_destroy(&aw8624->work_lock);
