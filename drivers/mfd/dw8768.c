...
int dw8768_register_set(u8 address, u8 value)
{
	...
	if (dw8768_base == NULL) {
		pr_err("[Display][%s] invalid dw8768 address \n", __func__);
		return -ENODEV;
	}
...
int dw8768_set_output_voltage(uint8_t val)
{
	...
	if (dw8768_base == NULL) {
		pr_err("[Display][%s] dw8768 was not probed\n", __func__);
		return -ENODEV;
	}
	...
	if (ret == 0) {
		...
		if (ret < 0) {
			pr_err("[Display][%s] DW8768_VPOS_VOLTAGE_REG read failed\n", __func__);
			return ret;
		} else {
			pr_info("[Display][%s] DW8768_VPOS_VOLTAGE_REG = 0x%02X\n", __func__, ret);
			ret = 0;
		}
	} else {
		pr_err("[Display][%s] DW8768_VPOS_VOLTAGE_REG write failed\n", __func__);
		return ret;
	}
	...
	if (ret == 0) {
		...
		if (ret < 0) {
			pr_err("[Display][%s] DW8768_VNEG_VOLTAGE_REG read failed\n", __func__);
			return ret;
		} else {
			pr_info("[Display][%s] DW8768_VNEG_VOLTAGE_REG = 0x%02X\n", __func__, ret);
			ret = 0;
		}
	} else {
		pr_err("[Display][%s] DW8768_VNEG_VOLTAGE_REG write failed\n", __func__);
		return ret;
	}
...
static int dw8768_probe(struct i2c_client *cl, const struct i2c_device_id *id)
{
	...
	rc = mfd_add_devices(dev, -1, dw8768_devs, ARRAY_SIZE(dw8768_devs),
			       NULL, 0, NULL);
	if (rc) {
		pr_err("[Display][%s] Failed to add dw8768 subdevice ret=%d\n", __func__, rc);
		return -ENODEV;
	}
