...
static int exfat_remount(struct super_block *sb, int *flags, char *data)
{
	...
	*flags |= SB_NODIRATIME;
	...
	exfat_msg(sb, KERN_INFO, "re-mounted(%s->%s), Opts: %s",
		(prev_sb_flags & SB_RDONLY) ? "ro" : "rw",
		(*flags & SB_RDONLY) ? "ro" : "rw", orig_data);
	kfree(orig_data);
	return 0;
}
#endif
			...
			struct exfat_mount_options *opts)
{
	...
	while ((p = strsep(&options, ","))) {
		...
		switch (token) {
			...
			case Opt_namecase:
			case Opt_codepage:
				/* these are deprecated options but do not spit error out */
				break;
			default:
				if (!silent)
...
static int exfat_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	err = parse_options(data, silent, &sbi->options);
	if (err)
		goto out_option_fail;
